...
//  نسخة محسّنة مع الاستهلاك الشهري
// ════════════════════════════════════════════════════

// ── DATABASE ──────────────────────────────────────
const db = new Dexie('ReorderPointDB');
db.version(2).stores({
    materials: '++id, itemCode, itemName, currentStock, reorderPoint, monthlyConsumption, leadTime, lastUpdated'
});
...
function renderTable(materials, tbody) {
    tbody.innerHTML = materials.map((m, idx) => {
        ...
        // حساب الكمية المقترحة بناءً على الاستهلاك الشهري ومدة التوريد
        const consumptionDuringLead = (m.monthlyConsumption || 0) * (m.leadTime / 30);
        const safetyStock = m.reorderPoint || 0;
        const recommendedQty = needsReorder
            ? Math.max(0, consumptionDuringLead + safetyStock - m.currentStock).toFixed(0)
            : null;
        ...
        <tr class="${needsReorder ? 'row-danger' : ''}" style="animation-delay:${idx * 0.04}s">
            ...
            <td style="text-align:center">
                <span class="consumption-badge">${formatNum(m.monthlyConsumption || 0)}</span>
            </td>
    ...
    container.innerHTML = materials.map((m, idx) => {
        ...
        <div class="grid-item ${needsReorder ? 'danger-item' : ''}" style="animation-delay:${idx * 0.05}s">
            ...
            <div class="grid-stats">
                ...
                <div class="grid-stat-box">
                    <div class="grid-stat-label">الاستهلاك الشهري</div>
                    <div class="grid-stat-val" style="color:var(--accent-secondary)">${formatNum(m.monthlyConsumption || 0)}</div>
                </div>
            </div>
            ${recommendedQty !== null ? `
...
async function showAlertView() {
    ...
    if (alerts.length === 0) {
        ...
        el.innerHTML = alerts.map((m, idx) => {
            const consumptionDuringLead = (m.monthlyConsumption || 0) * (m.leadTime / 30);
            const safetyStock = m.reorderPoint || 0;
            const recommendedQty = Math.max(0, consumptionDuringLead + safetyStock - m.currentStock).toFixed(0);
            ...
            <div class="alert-item" style="animation-delay:${idx * 0.06}s">
                ...
                <div class="alert-stats">
                    ...
                    <div class="alert-stat-box">
                        <div class="alert-stat-label">الاستهلاك الشهري</div>
                        <div class="alert-stat-val orange">${formatNum(m.monthlyConsumption || 0)}</div>
                    </div>
                    <div class="alert-stat-box">
...
async function exportAlerts() {
    ...
    const rows = alerts.map(m => {
        const consumptionDuringLead = (m.monthlyConsumption || 0) * (m.leadTime / 30);
        const safetyStock = m.reorderPoint || 0;
        const recommendedQty = Math.max(0, consumptionDuringLead + safetyStock - m.currentStock);
        ...
        return {
            ...
            'الاستهلاك الشهري': m.monthlyConsumption || 0,
            'الكمية المطلوبة': recommendedQty,
...
async function editMaterial(id) {
    try {
        ...
        document.getElementById('monthlyConsumption').value = m.monthlyConsumption ?? 0;
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (cols.length >= 6 && cols[0]) {
            materials.push({
                ...
                monthlyConsumption: parseFloat(cols[4]) || 0,
                leadTime          : parseInt(cols[5])   || 0,
                lastUpdated       : new Date()
            });
        }
    }
...
function parseExcelData(data) {
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        materials.push({
            ...
            monthlyConsumption: parseFloat(row[4]) || 0,
            leadTime          : parseInt(row[5])   || 0,
            lastUpdated       : new Date()
        });
    }
    return materials;
...
async function exportData() {
    ...
    const rows = all.map(m => {
        const consumptionDuringLead = (m.monthlyConsumption || 0) * (m.leadTime / 30);
        const safetyStock = m.reorderPoint || 0;
        const recommendedQty = m.currentStock <= m.reorderPoint 
            ? Math.max(0, consumptionDuringLead + safetyStock - m.currentStock) 
            : 0;
        ...
        return {
            ...
            'الاستهلاك الشهري': m.monthlyConsumption || 0,
            'مدة التوريد (يوم)': m.leadTime,
            'الكمية المقترحة' : recommendedQty,
